...
FactoryBot.define do
  factory :work, aliases: [:generic_work, :private_generic_work], class: Work do
    transient do
      user { create(:user) }
    ...
    factory :work_with_public_image_files do
      before(:create) { |work, evaluator| 2.times { work.ordered_members << create(:file_set, :public, :image, user: evaluator.user) } }
    end
    ...
    factory :work_with_public_representative_file do
      before(:create) do |work, evaluator|
        work.ordered_members << create(:file_set, :public, user: evaluator.user, title: ['A Contained FileSet'])
        work.representative_id = work.members[0].id
      end
    end
